...
namespace EaseFilter.FilterControl
{
    static public class WinData
    {
        ...
        public static string DesiredAccessToString(uint desiredAccess)
        {
            string ret = string.Empty;

            foreach (DisiredAccess access in Enum.GetValues(typeof(WinData.DisiredAccess)))
            {
            ...
            {
                ret = "(0x)" + desiredAccess.ToString("X");
            }

            return ret;
        }

        public static string CreateOptionsToString(uint createOptions)
        {
            string ret = string.Empty;

            foreach (CreateOptions option in Enum.GetValues(typeof(WinData.CreateOptions)))
            {
