...
namespace AutoFileCryptTool
{
    public partial class Form_FileCrypt : Form
    {
        ...
        public void SendConfigSettingsToFilter()
        {
            ...
            FileFilterRule dropFolderFilterRule = new FileFilterRule();
            dropFolderFilterRule.IncludeFileFilterMask = GlobalConfig.DropFolder + "\\*";
            //enable the encryption, it is for decryption, disable encrypt new file since the file was encrypted in this folder.
            dropFolderFilterRule.AccessFlag = (FilterAPI.ALLOW_MAX_RIGHT_ACCESS | (uint)FilterAPI.AccessFlag.ENABLE_FILE_ENCRYPTION_RULE) & (~(uint)FilterAPI.AccessFlag.ALLOW_ENCRYPT_NEW_FILE);
            dropFolderFilterRule.EncryptionPassPhrase = GlobalConfig.MasterPassword;
            dropFolderFilterRule.EncryptMethod = (int)FilterAPI.EncryptionMethod.ENCRYPT_FILE_WITH_SAME_KEY_AND_UNIQUE_IV;

            FileFilter dropFolderFileFilter = dropFolderFilterRule.ToFileFilter();
